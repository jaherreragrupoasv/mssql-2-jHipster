var fs = require('fs');
var sequelize_class = require('./sequelize_class');
var sequelize_routes = require('./sequelize_routes');
    ...
    connection.on('connect', function (err) {
        async.waterfall([
            ...
            function (callback) {
                ...
                var request = new Request(
                    "SELECT TOP 1 TABLE_NAME FROM information_schema.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_SCHEMA = 'Person'",
                    //"SELECT DB_NAME()",
                ...
                async.forEachSeries(
                    ...
                    function (schema, callback) {
                        try {
                            ...
                            //RUTAS
                            var bufferRoutes = sequelize_routes(schema);

                            if (filedir != null) {
                                var filename = filedir + '/route' + schema.table + '.js';

                                fs.open(filename, 'wx', 0644, function (err, fd) {
                                    if (!err) {
                                        fs.write(fd, bufferRoutes, function (err, written, string) {
                                ...
                                console.log(bufferRoutes);
                                //callback(null);
                            }

                            //CLASES
                            var bufferClasses = sequelize_class(schema);
                            ...
                            if (filedir != null) {
                                ...
                                fs.open(filename, 'wx', 0644, function (err, fd) {
                                    if (!err) {
                                        fs.write(fd, bufferClasses, function (err, written, string) {
                                            fs.close(fd, function (err) {
                                                console.log(filename + ": created.");
                                                callback(null);
                                            });
