var fs = require('fs');
var jHipsterClass = require('./jHipsterClass');
    ...
    connection.on('connect', function (err) {
        async.waterfall([
            ...
            function (callback) {
                ...
                //"SELECT TOP 1 TABLE_NAME FROM information_schema.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_SCHEMA LIKE '%'",
                //"SELECT DB_NAME()",
                // "SELECT name from Sys.Databases",

                //"SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_NAME LIKE 'ACTUARIAL_ALTASF%'",
            ...
            function (schemas, callback) {
                async.forEachSeries(
                    ...
                    function (schema, callback) {
                        try {
                            var buffer = jHipsterClass(schema);

                            if (filedir != null) {
                                var filename = filedir + '/' + schema.table + '.jh';
